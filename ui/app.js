...
const DEFAULT_GLASS_ALPHA_PCT = 70;
...
function applyGlassVars(alphaPct) {
...
function applyTheme(theme) {
  ...
  if (themeBtn) {
    themeBtn.textContent =
      t === 'dark' ? '暗色（当前）/ 浅色' : '暗色 / 浅色（当前）';
    themeBtn.setAttribute('aria-pressed', t === 'dark' ? 'true' : 'false');
    themeBtn.title = t === 'dark' ? '当前暗色 · 点击切换浅色' : '当前浅色 · 点击切换暗色';
  }
}
...
function applySettingsState(settings) {
  ...
  applyGlassVars(appSettings.glassAlpha);
