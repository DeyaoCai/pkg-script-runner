...
const UI_DIST_INDEX = path.join(APP_ROOT, 'dist-ui', 'index.html');
...
async function loadMainWindow(win: BrowserWindow): Promise<void> {
  ...
  if (fs.existsSync(UI_DIST_INDEX)) {
    await win.loadFile(UI_DIST_INDEX);
    return;
  }
  await win.loadFile(path.join(APP_ROOT, 'ui', 'index.vanilla.html'));
}
...
/** UI settings from tray-owned shared-settings.json */
let shared: SharedSettings = loadSharedSettings();
...
function applySharedToRuntime(next: SharedSettings): void {
  ...
  if (logsChanged) setPersistLogs(shared.persistLogs);
...
function applySettingsPatch(patch: Partial<AppSettings>): {
  ...
  const settings = patchSharedSettings(patch);
  applySharedToRuntime(settings);
  return { settings, hotkeyError: null };
}

function openSharedSettingsUi(): void {
  requestTrayOpenSettings();
  // Nudge tray process (dev or packaged) so settings open even if watcher missed.
  try {
    const trayDir = path.join(APP_ROOT, '..', 'tray');
    const electronBin = process.execPath;
    if (fs.existsSync(path.join(trayDir, 'package.json'))) {
      spawn(electronBin, ['.', '--open-settings'], {
        cwd: trayDir,
        detached: true,
        stdio: 'ignore',
        env: process.env,
...
function setPersistLogsPref(enabled: boolean) {
  applySettingsPatch({ persistLogs: enabled });
  if (enabled) {
...
function registerIpc() {
  ...
  ipcMain.handle('pkg:get-settings', () => shared);
  ...
  ipcMain.handle('pkg:set-persist-logs', (_e, enabled: boolean) => {
    ...
    return shared.persistLogs;
  });

  ipcMain.handle('pkg:open-shared-settings', () => {
    openSharedSettingsUi();
  });
...
if (gotSingleInstanceLock) {
  ...
  app.whenReady().then(() => {
    ...
    stopSharedWatch = watchSharedSettings((next) => applySharedToRuntime(next));
